...
export const login = createAsyncThunk(
  ...
  async ({ emailOrPhone, password }, { rejectWithValue }) => {
    try {
      ...
      const message =
        error.response?.data?.message || error?.response?.data?.error[0].msg;
...
export const loginFacebook = createAsyncThunk(
  ...
  async (token, { rejectWithValue }) => {
    try {
      const response = await axiosClient.post("/api/facebook-login", {
        accessToken: token,
      });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(register.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload || action.error.message || "Regisser Failed";
      })
